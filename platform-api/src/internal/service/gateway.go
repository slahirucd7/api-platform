...
func (s *GatewayService) SyncManifest(gatewayID, orgID string) (*ManifestJob, error) {
	...
	// Trigger a new request when there is no active job, the previous one failed, or timed out
	if status == nil || *status == "failed" {
		...
		if err := s.gatewayEventsService.BroadcastGatewayManifestRequestEvent(gatewayID, correlationID); err != nil {
			s.slogger.Warn("Failed to send gateway manifest request event — gateway may be offline",
				slog.String("gateway_id", gatewayID),
				slog.Any("error", err),
			)
			if dbErr := s.gatewayRepo.UpdateManifestJob(gatewayID, "failed"); dbErr != nil {
				s.slogger.Error("Failed to mark manifest job as failed in DB",
					slog.String("gateway_id", gatewayID),
					slog.Any("error", dbErr),
				)
			}
			return &ManifestJob{GatewayID: gatewayID, Status: "failed"}, nil
		}

		return &ManifestJob{GatewayID: gatewayID, Status: "pending"}, nil
	}

	// Already pending and within timeout — don't re-trigger
	if *status == "pending" {
		return &ManifestJob{GatewayID: gatewayID, Status: "pending"}, nil
	}
